...
function jlmie_Qsca_n(m, x, n::Int)
    ...
    nth_Qsca = 2 ./ x.^2 .* (2n + 1) .* (abs.(an).^2 .+ abs.(bn).^2)
    return nth_Qsca
end

"""
    jlmie_Qsca_a_n(m, x, n::Int)
...
function jlmie_Qsca_a_n(m, x, n::Int)
    ...
    nth_Qsca_a = 2 ./ x.^2 .* (2n + 1) .* (abs.(an).^2)
    return nth_Qsca_a
end
"""
    jlmie_Qsca_b_n(m, x, n::Int)
...
function jlmie_Qsca_b_n(m, x, n::Int)
    ...
    nth_Qsca_b = 2 ./ x.^2 .* (2n + 1) .* (abs.(bn).^2)
    return nth_Qsca_b
end